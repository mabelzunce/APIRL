/**
	\file MLEM.cpp
	\brief Executable for MLEM reconstruction.

	This is the main source file for the MLEM executable. The executable gets as input parameter a file
	with the configuration of the different parameters needed. The parameters file is in interfile format
	and below can be found an example.

	\par Example of MLEM parameter file reconMLEM.par:
	\code
	    MLEM Parameters :=
...
int main (int argc, char *argv[])
{
  ...
  {
    cout<< strForwardprojector << " is not valid projector. " << endl;
    return -1;
  }
  ...
  {
    cout<< strBackprojector << " is not valid backprojector. " << endl;
    return -1;
  }
